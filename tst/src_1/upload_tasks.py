from initial_process import store_json_data, read_json_data, force_py
...
async def safe_transfer_old_record(env, mobile, pri_id):
    ...
    tgt = f"mobile/{mobile}/transferred/{pri_id}.json"
...
async def r2_bucket_push(env, records):
    ...
    for record in records:
        ...
        if customer_id:
            ...
            upload_tasks.append(store_json_data(env, record, cus_id_key, metadata))

        if email:
            # (POS_BUCKET)
            email_key = f"{mobile_prefix}/{mobile}/email/{email}.json"
            upload_tasks.append(store_json_data(env, record, email_key, metadata))
