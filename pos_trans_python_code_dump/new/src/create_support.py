...
async def fetch_age_for_serials(env, serials: set):
    ...
    async def safe_fetch(sn):
        async with semaphore:
            try:
                age = await asyncio.wait_for(fetch_age_of_device(env, sn), timeout=AGE_API_TIMEOUT)
                results[sn] = age
            except asyncio.TimeoutError:
                console.error(f"[AGE] ⏱ Timeout | sn={sn}")
                results[sn] = None
            except Exception as e:
...
async def r2_bucket_push(env, records):
    ...
    if mobile and all_records:
        try:
            ...
            console.error(f"[CREATE] ❌ Failed to summary queue batch {batch_id}: {e}")
