...
async def safe_replace_old_record(env, mobile, pri_id):
    ...
    await store_json_data(env, old_data, trans_key, bucket_binding="POS_BUCKET_PROD")
...
async def update_mobile_summary(env, mobile, new_records):
    ...
    key = f"mobile/{mobile}/mobile_summary.json"
    await store_json_data(env, summary_data, key, summary_meta, bucket_binding="POS_BUCKET_PROD")
