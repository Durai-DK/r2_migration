import uuid, random, json, asyncio
...
async def move_to_p_failure(env, batch_id, invoice, error, key):
    ...
    await store_json_data(env, invoice, key, p_fail_meta, bucket_binding="POS_BUCKET_PROD")
...
async def handle_permanent_failure(env, batch_id, payload, error, queue):
    ...
    failure_data = {**(payload or {})}
    failure_data["dump_at"] = datetime.utcnow().isoformat()

    await move_to_p_failure(env, batch_id, failure_data, error, key)

    console.error(f"[{queue}][PERMANENT] ❌ batch_id:{batch_id} | error:{error}")
...
async def failed_queue_store(env, payload, batch_id, e, status):
    ...
    await store_json_data(env, data, key, metadata, bucket_binding="POS_BUCKET_PROD")
