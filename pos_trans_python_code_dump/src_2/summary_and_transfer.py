...
async def sync_verified_records(env, mobile: str, old_records: list, new_records: list):
    ...
    new_verified_keys = {
        verified_keys(r) for r in new_records if record_is_valid(r)
    }
    ...
    for old in old_records:
        ...
        if verified_keys(old) in new_verified_keys:
...
def merged_summary_records(old_records, new_records):
    """
    Merge old + new summary records with rules:
    1. If NEW verified identity exists → remove matching OLD verified record
    2. Deduplicate by pri_id
    3. NEW always wins
    """

    # 🔑 Step 1: Collect verified identities from NEW records
    new_verified_identities = {
        verified_keys(r) for r in new_records if record_is_valid(r)
    }
    ...
    filtered_old = []
    for old in old_records:
        if record_is_valid(old):
            if verified_keys(old) in new_verified_identities:
                continue  # replaced by new verified record
        filtered_old.append(old)

    # 🧠 Step 3: Deduplicate by pri_id (new always wins)
    index = {}

    for r in filtered_old:
        pri_id = r.get("pri_id")
        if pri_id:
            index[pri_id] = r
