...
async def handle_get_records(request, env):
    ...
    if imei:
        ...
        if category == "imei_history":
            return await imei_history(env.IMEI_BUCKET_LIVE, key, request=request)

        return await fetch_single_imei(env.IMEI_BUCKET_LIVE, key, request=request)
    ...
    if mobile:
        ...
        if s_id:
            ...
            return await fetch_specific_file(env.POS_BUCKET_LIVE, key, request=request)
        ...
        if category == "mobile_summary":
            return await summary_retrieve(env.POS_BUCKET_LIVE, category_key, page_no, per_page, start_date, end_date, request=request)

        if category == "unique_invoices":
            return await unique_invoices(env.POS_BUCKET_LIVE, category_key, start_date, end_date, request=request)

        if category == "invoice":
            return await invoice_retrieve(env.POS_BUCKET_LIVE, category_key, invoice_no, request=request)

        if category == "imei":
            return await imei_retrieve(env.POS_BUCKET_LIVE, category_key, invoice_no, request=request)
