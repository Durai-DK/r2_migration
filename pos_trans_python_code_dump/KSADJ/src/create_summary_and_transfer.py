...
def identity_key(record):
    mobile = (record.get("customer_mobile__c") or record.get("Customer_Number__c") or record.get("customer_phone_no2") or "")
...
async def rebuild_imei_summary(env, batch_id, imei):
    if not imei:
        console.log(f"[SUMMARY] ℹ️ No IMEIs to process | batch_id={batch_id}")
        return
    ...
    for attempt in range(4):
        try:
            # Small jitter to reduce collision burst
            await asyncio.sleep(random.uniform(0.01, 0.05))

            # 1️⃣ Get current summary with ETag
            existing_obj = await env.IMEI_BUCKET_PROD.get(summary_key)
            existing_etag = existing_obj.etag if existing_obj else None

            # 2️⃣ List all ID records
            listed = await env.IMEI_BUCKET_PROD.list(prefix=prefix)
            keys = [obj.key for obj in listed.objects]

            # No records → skip write
            if not keys:
                console.log(f"[IMEI] ℹ️ No records found | imei={imei}")
                return

            # 3️⃣ Load all files concurrently
            ...
            metadata_js = Object.fromEntries([["serial_no", str(imei)], ["records-count", str(len(final_records))],])

            http_meta_js = Object.fromEntries([["contentType", "application/json"]])

            # 4️⃣ Optimistic locking write
            if existing_etag:
                await env.IMEI_BUCKET_PROD.put(
                    summary_key, body, customMetadata=metadata_js, httpMetadata=http_meta_js, onlyIf={"etagMatches": existing_etag},
                )
            else:
                await env.IMEI_BUCKET_PROD.put(
                    summary_key, body, customMetadata=metadata_js, httpMetadata=http_meta_js,
                )

            console.log(f"[IMEI] ✅ Summary updated | imei={imei}")
            return

        except Exception as e:
            # ETag conflict → retry
            if "etag" in str(e).lower():
                console.warn(f"[IMEI] ⚠️ ETag conflict | imei={imei} | retry={attempt + 1}")
                await asyncio.sleep(0.05 * (attempt + 1))
                continue

            # R2 concurrency error → retry
            if "Reduce your concurrent request rate" in str(e):
                console.warn(f"[IMEI] ⚠️ R2 rate limit | imei={imei} | retry={attempt + 1}")
                await asyncio.sleep(0.1 * (attempt + 1))
                continue

            raise

    raise TemporaryError(f"IMEI summary update failed after retries | imei={imei}")
