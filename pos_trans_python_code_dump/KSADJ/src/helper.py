...
async def store_json_data(env, data, key, metadata, bucket_binding):
    if not key:
...
async def read_json_data(env, key, bucket_binding):
    try:
        bucket = getattr(env, bucket_binding, None)
        if not bucket:
            return None
...
def clean_email(raw_email: str) -> str:
    ...
    if "@" not in email and email.endswith("gmail.com"):
