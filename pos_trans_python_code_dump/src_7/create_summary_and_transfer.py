...
async def run_summary_logic(env, mobile, records, serials):
    ...
    async def safe_rebuild(imei):
        async with semaphore:
            try:
                ...
                console.error(f"[IMEI] ❌ Failed | {imei} | {str(e)}")
                failed_imeis.append(imei)

    await asyncio.gather(*[safe_rebuild(i) for i in serials])

    # Requeue only failed IMEIs
    if failed_imeis:
        await env.POS_SUMMARY_QUEUE.send({
            "type": "imei", "serial_numbers": failed_imeis, "replay_count": 1}, {"delaySeconds": 10}
        )


async def handle_full_summary(env, payload):
    ...
    # -------- Core Logic --------
    try:
        await asyncio.wait_for(run_summary_logic(env, mobile, records, serials), timeout=TIMEOUT)
    except asyncio.TimeoutError:
        raise TemporaryError("Summary timeout")
...
async def handle_mobile_only(env, payload):
    mobile = payload.get("mobile")
    records = payload.get("records") or []
    ...
    await update_mobile_summary(env, mobile, records)
    console.log(f"[SUMMARY_QUEUE] ✅ Mobile Only Completed | {mobile}")


async def handle_imei_only(env, payload):
    serials = list(set(payload.get("serial_numbers") or []))
    if not serials:
        raise PermanentError("Missing serial numbers")
    ...
    await asyncio.gather(*[safe_rebuild(i) for i in serials])
    console.log(f"[SUMMARY_QUEUE] ✅ IMEI Only Completed | count={len(serials)}")
