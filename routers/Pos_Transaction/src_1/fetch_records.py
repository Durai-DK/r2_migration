...
async def handle_get_records(request, env):
    ...
    if imei:
        ...
        return await fetch_specific_file(env.IMEI_BUCKET_LIVE, key)
    ...
    if mobile:
        ...
        if s_id:
            ...
            return await fetch_specific_file(env.POS_BUCKET_LIVE, key)
        ...
        if category == "mobile_summary":
            return await summary_retrieve(env.POS_BUCKET_LIVE, category_key, page_no, per_page, start_date, end_date)

        if category == "invoice":
            return await invoice_retrieve(env.POS_BUCKET_LIVE, category_key, invoice_no)

        if category == "imei":
            return await imei_retrieve(env.POS_BUCKET_LIVE, category_key, invoice_no)
