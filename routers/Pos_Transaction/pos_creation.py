...
def verify_and_archive_record(bucket: str, key: str, new_record: Dict[str, Any], mobile: str, imei: str, mobile_prefix: str, pre_fetched_summary_record: Optional[Dict[str, Any]] = None):
...
def generate_upload_tasks(invoice: Dict[str, Any], pri_id: str, pos_bucket: str, mobile_prefix: str, imei_prefix: str, old_summary_record: Optional[Dict[str, Any]] = None):
    ...
    for record in records:
        ...
        tasks.append((verify_and_upload_imei_record, (pos_bucket, mobile_imei, record, mobile, serial_no, metadata, imei_prefix, mobile_prefix, old_summary_record)))
...
def verify_and_upload_imei_record(bucket, mobile_imei_key, record, mobile, serial_no, metadata, imei_pfx, mobile_pfx, old_summary_record=None):
    # Verify and Archive
    verify_and_archive_record(bucket, mobile_imei_key, record, mobile, serial_no, mobile_pfx, pre_fetched_summary_record=old_summary_record)
...
def rebuild_mobile_summary(bucket: str, mobile: str, mobile_prefix: str):
    """
...
def push_to_bucket(invoice: Dict[str, Any], pos_bucket: str, imei_bucket: str, mobile_prefix: str, imei_prefix: str):
    # This function handles a single invoice but uses parallelism for its items (IMEIs)
    pri_id = invoice.get("pri_id") or invoice.get("id") or invoice.get("bill_transaction_no__c") or "unknown"
    pri_id = str(pri_id)
    
    tasks = generate_upload_tasks(invoice, pri_id, pos_bucket, mobile_prefix, imei_prefix)
    ...
    try:
        ...
        if mobile:
            rebuild_mobile_summary(pos_bucket, mobile, mobile_prefix)
