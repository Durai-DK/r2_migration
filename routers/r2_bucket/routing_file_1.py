...
def fetch_files(
                    ...
                    imei_or_mobile: str = Query(..., description="IMEI or Mobile number"),
                    category: str = Query(None, enum=["specific_file", "mobile_summary"]),
                    specific_field: str | None = Query(None, enum=["id", "imei"]),
                    specific_id: str | None = None,

                    summary_field: str | None = Query(None, enum=["invoice", "imei"]),
    ...
    # ----- Validation -----
    if field_type == "imei":
        if not imei_or_mobile:
            return error_response(400, "imei_no is missing", "imei_no is required to fetch IMEI data")
        ...
        if category == "mobile_summary":
            if summary_field == "imei" and not invoice_no:
                return error_response(400, "invoice_no is missing", "invoice_no is required to fetch IMEI list.")
    ...
    if field_type == "imei" and imei_no:
        ...
        if category == "mobile_summary":
            ...
            if summary_field == "invoice":
                key = f"{mob_prefix}/{mobile_no}/mobile_summary.json"
                return invoice_retrieve(mob_bucket, key, page_no, per_page, start_date, end_date)

            elif summary_field == "imei":
                key = f"{mob_prefix}/{mobile_no}/mobile_summary.json"
                return imei_retrieve(mob_bucket, key, invoice_no)
