...
#
#
# def store_json_summary_data(bucket, data, key, metadata):
#
#     if metadata is None:
#         metadata = {}
#     if not key:
...
#     # try:
#     #     buffer = BytesIO()
#     #     with gzip.GzipFile(filename="", mode="wb", fileobj=buffer) as gz:
#     #         gz.write(json_body.encode("utf-8"))
#     #     compressed_body = buffer.getvalue()
#     #
#     # except Exception as e:
#     #     logger.error(f"❌ GZIP compression failed for key {key}: {e}")
#     #     raise
#
#     try:
#         r2 = r2_client()
...
#                       Metadata=metadata)
#
#         logger.info(f"✅ Stored {key} ({len(gzipped)} bytes)")
#
#     except Exception as e:
#         logger.exception(f"❌ Error storing {key}: {e}")
#         raise
#
...
#
#     imei_summary = {"records_count": len(records), "records": records}
#     metadata = {"mobile": mobile, "records_count": str(len(records))}
#
#     # store_json_summary_data(bucket, imei_summary, imei_summary_key, metadata)
...
#     id_summary = {
#         "records_count": len(records), "total_spends": total_spends, "records": records
#     }
#     metadata = {
#         "mobile": mobile, "records_count": str(len(records)), "total_spends": str(total_spends)
#     }
#
#     # store_json_summary_data(bucket, invoice_summary, invoice_summary_key, metadata)
