...
def header_record_mapping(data):
    ...
    records = {
        ...
        'item_serial_no__c': data.get("item_serial_no", ""),
    }
    return records
...
async def process_record(invoice, env, pri_id):
    ...
    # Ensure item_remarks1__c is prioritized from the footer or header if missing
    if not record.get("item_remarks1__c"):
        record["item_remarks1__c"] = invoice.get("item_remarks1", "")
...
async def transfer_old_record(env, mobile, pri_id):
    src = f"mobile/{mobile}/id/{pri_id}.json"
    dst = f"mobile/{mobile}/transfered/{pri_id}.json"

    old_data = await read_json_data(env, src)
    if not old_data:
        return

    await store_json_data(env, old_data, dst)
    await env.POS_BUCKET.delete(src)

    console.log(f"🔁 Transferred old record: {pri_id}")
...
async def generate_upload_tasks(record, env, pri_id):
    ...
    serial_raw = str(base_record.get("item_serial_no__c") or "")
    serial_nos = [s.strip() for s in serial_raw.split(",") if s.strip()]

    single_serial_age = None
    if len(serial_nos) == 1:
        single_serial_age = await fetch_age_of_device(serial_nos[0], env)
        base_record["ageOfDevice"] = single_serial_age

    base_metadata = {
        "customer-id": str(customer_id or ""),
        "invoice-id": str(invoice_id or ""),
        "mobile": str(mobile or ""),
        "serial-no": str(serial_raw or ""),
        "pri_id": str(pri_id or ""),
        "email": str(email or ""),
    }

    # 🔁 Transfer old verified records
    if mobile:
        verified_pri_ids = await get_verified_pri_ids_from_summary(env, mobile)
        if verified_pri_ids:
            await asyncio.gather(*[
                transfer_old_record(env, mobile, old_pri_id)
                for old_pri_id in verified_pri_ids
                if str(old_pri_id) != str(pri_id)
            ])

    # Store ID file (POS_BUCKET)
    mobile_pri_id_key = f"{mobile_prefix}/{mobile}/id/{pri_id}.json"
    upload_tasks.append(store_json_data(env, base_record, mobile_pri_id_key, base_metadata))

    # History (IMEI_BUCKET)
    history_key = f"history/{year}/{month}/{day}/{pri_id}.json"
    upload_tasks.append(store_json_data(env, base_record, history_key, base_metadata, bucket_binding="IMEI_BUCKET"))

    if customer_id:
        # (POS_BUCKET)
        cus_id_key = f"{mobile_prefix}/{mobile}/cus_id/{customer_id}.json"
        upload_tasks.append(store_json_data(env, base_record, cus_id_key, base_metadata))

    if email:
        # (POS_BUCKET)
        email_key = f"{mobile_prefix}/{mobile}/email/{email}.json"
        upload_tasks.append(store_json_data(env, base_record, email_key, base_metadata))

    ages = await asyncio.gather(
        *[fetch_age_of_device(sn, env) for sn in serial_nos]
    )

    for sn, age in zip(serial_nos, ages):
        ...
        upload_tasks.append(store_json_data(env, sn_record, key_1, imei_metadata))

        # 2. imei/sn (IMEI_BUCKET)
        key_2 = f"{imei_prefix}/{sn}.json"
        upload_tasks.append(store_json_data(env, sn_record, key_2, imei_metadata, bucket_binding="IMEI_BUCKET"))
