...
import requests, os, json, time, concurrent.futures
...
def generate_upload_tasks(record, pri_id):
    ...
    if mobile:
        verified_pri_ids = get_verified_pri_ids_from_summary(mobile)
        for old_pri_id in verified_pri_ids:
            if str(old_pri_id) != str(pri_id):
                transfer_old_record(mobile, old_pri_id)

    serial_nos = [s.strip() for s in serial_raw.split(",") if s.strip()]

    single_serial_age = None
    if len(serial_nos) == 1:
        single_serial_age = fetch_age_of_device(serial_nos[0])
...
def push_to_bucket(invoice: Dict[str, Any]):
    ...
    try:
        ...
        if mobile:
            ...
            error_log.warning(f"⚠️ Could not rebuild summary, invalid mobile in invoice: {mobile}")
    except Exception as e:
        error_log.exception(f"❌ Failed to trigger summary rebuild in push_to_bucket: {e}")

    try:
        save_process_errors(pri_id, errors)
        success_log.info(f"🔄 error record stored {pri_id}")
    except Exception as e:
        error_log.exception(f"❌ Failed to store error record: {e}")