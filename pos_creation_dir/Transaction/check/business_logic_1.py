...
async def crm_customer_id(payload, env):
    ...
    try:
        ...
        data = {
            ...
            "mobile": str(payload.get("customer_mobile") or payload.get("customer_phone_no1") or payload.get(
                "customer_phone_no2")),
            "gender": str(payload.get("customer_gender")),
            "email": str(payload.get("customer_email")),
            "type": "Customer",
            "pincode": str(payload.get("customer_pincode") or payload.get("billed_at_pincode") or payload.get(
                "delivery_to_pincode")),
            "branch": [str(payload.get("store_code"))],
            "city": str(
                payload.get("customer_city") or payload.get("billed_at_city") or payload.get("delivery_to_city")),
            "state": str(
                payload.get("customer_state") or payload.get("billed_at_state") or payload.get("delivery_from_state")),
...
async def generate_upload_tasks(record, env, pri_id):
    ...
    mobile = str(base_record.get("customer_phone_no1", "")).strip()
    if not mobile:
        console.warn(f"⚠️ Missing mobile for pri_id={pri_id}")
        return
    ...
    for old_pri_id in verified_pri_ids:
        if str(old_pri_id) != str(pri_id):
            await transfer_old_record(env, mobile, old_pri_id)
    ...
    for sn in serial_nos:
        ...
        upload_tasks.append(
            store_json_data(env, sn_record, key_2, metadata=imei_metadata, bucket_binding="IMEI_BUCKET"))
